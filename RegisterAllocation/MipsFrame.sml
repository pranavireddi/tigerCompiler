...
ahhhh soooo many complex connections lol. *)
    ...
    struct
        ...
        datatype frag =
            ProcFrag of {body: Tr.stm, frame: frame}
        | StringFrag of {label: Temp.label, str: string}

        val saveArea = 40   (* 0:ra, 4:fp, 8..36:s0..s7 *)
        val startOffset = saveArea
        ...
        (* First k args arrive in registers.
        Escaping ones get homes in this frame at positive FP offsets.
        Non-escaping ones stay in temps.
        Args beyond k are stack-passed by caller. We model them as being above
        this frame; this convention may need adjustment if your caller layout changes. *)

        fun allocLocal (frame : frame) escape =
            if escape then
                let
                    val offset = !(#localOffset frame)
                    val _ = (#localOffset frame) := offset + wordSize
                in
                    InFrame offset
                end
            else
        ...
        fun externalCall (func, args) =
            Tr.CALL (Tr.NAME (Temp.namedLabel ("tig_" ^ func)), args)
        ...
        fun procEntryExit1 ({formals, ...} : frame, body) =
         ...
         fun mipsInt n =
            if n < 0 then "-" ^ Int.toString (Int.abs n)
            else Int.toString n
        ...
        fun procEntryExit3 ({name, localOffset, ...} : frame, body) =
            let
        ...
        let
            ...
            fun alloc (esc, idx) =
                if idx < k then
                    ...
                    (* stack-passed args: leave separate for now *)
                    InFrame (!nextLocal + (idx - k) * wordSize)
